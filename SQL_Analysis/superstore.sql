...
Цель проекта
------------
Этот проект демонстрирует последовательный SQL-анализ датасета Superstore.
Задача состоит не только в расчёте KPI, но и в ответах на типичные бизнес-вопросы:

1. Какую выручку и прибыль получает бизнес?
2. Какие товары, категории, регионы и клиентские сегменты работают лучше или хуже?
3. Как скидки влияют на выручку и прибыльность?
4. Чем отличаются клиенты по частоте заказов и объёму выручки?
5. Как меняются показатели во времени?
6. Где оконные функции полезны для ранжирования, анализа роста и накопительных показателей?

Подход к анализу
----------------
Анализ начинается с проверки данных и базовых KPI, затем переходит к сегментации,
анализу клиентов, оконным функциям, ABC-анализу и когортам.
В каждом запросе поясняется, ЧТО мы считаем, ЗАЧЕМ это делаем
и КАК именно SQL реализует расчёт.

Важные замечания по качеству данных
-----------------------------------
- Поле `regoin`, вероятно, является опечаткой в слове `region`. Оно оставлено
  без изменений, чтобы скрипт соответствовал текущей структуре таблицы.
- Поле `discount` объявлено как INTEGER. Это может быть проблемой, если
  исходный файл хранит скидки как дробные значения, например 0.2 или 0.3.
  Для production-версии лучше использовать NUMERIC(5,2) или фактический тип источника.
- Путь в COPY привязан к локальному компьютеру. Перед публикацией на GitHub
  его нужно сделать переносимым или отдельно описать загрузку CSV.
- Выручка определяется как `SUM(sales)`. Поля `quantity`, `profit` и `discount`
  являются отдельными показателями и не складываются между собой для получения выручки.

================================================================================
*/

/* ШАГ 1 — СОЗДАНИЕ СТРУКТУРЫ ТАБЛИЦЫ
Цель: определить структуру, в которую будет загружен CSV-файл.
Как: для каждого поля датасета создаём отдельный столбец и назначаем подходящий
тип PostgreSQL. Даты храним как TIMESTAMP, количество — как INTEGER,
а sales и profit — как NUMERIC, поскольку это денежные показатели.
*/
...
Цель: импортировать исходный CSV-файл в PostgreSQL.
Как: COPY загружает данные напрямую в таблицу. HEADER пропускает строку
с названиями столбцов, DELIMITER задаёт разделитель, а ENCODING — кодировку файла.
Для публикации на GitHub локальный путь к файлу нужно сделать переносимым.
*/
...
Цель: убедиться, что таблица заполнилась, и посмотреть исходные записи.
Как: SELECT * выводит все строки и столбцы. В полноценном анализе после этого
проверяют пропуски, дубликаты, некорректные даты и неожиданные значения.
*/
